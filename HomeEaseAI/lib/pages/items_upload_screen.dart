...
class ItemsUploadScreen extends StatefulWidget {
  final VoidCallback? onUploadComplete; // ✅ new optional callback

  const ItemsUploadScreen({super.key, this.onUploadComplete});
...
class _ItemsUploadScreenState extends State<ItemsUploadScreen> {
  ...
  static Future<Map<String, String>> uploadImageToCloudinary(Uint8List imageBytes) async {
  ...
  saveItemInfoToFirestore() {
    ...
    if (widget.onUploadComplete != null) {
      widget.onUploadComplete!(); // ✅ go back to item list in HomeDecor
    }
  }
  ...
  Widget defaultScreen() {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Colors.white,
        elevation: 0,
        automaticallyImplyLeading: false, // We'll provide a custom back button
        title: Row(
          children: [
            IconButton(
            ...
            const SizedBox(width: 4), // 👈 adjust this to control spacing
            const Text(
              "Upload New Item",
              style: TextStyle(
                color: Color(0xFFCAA54D),
                fontWeight: FontWeight.w600,fontFamily:"FunnelDisplay",
                fontSize: 18,
              ),
            ),
          ],
      ...
      builder: (c) {
        return SimpleDialog(
          ...
          title: const Text(
            "Item Image",
            style: TextStyle(
              color: Color(0xFFCAA54D), // ✅ title font color
              fontWeight: FontWeight.bold,
              fontFamily: "FunnelDisplay",
            ),
          ),
          children: [
            SimpleDialogOption(
              ...
              child: const Text(
                "Capture with Camera",
                style: TextStyle(
                  color: Color(0xFFCAA54D), // ✅ option font color
                  fontFamily: "FunnelDisplay",
                ),
              ),
            ),
